...
let productNames = document.querySelector("#product-names");
// console.log(products);
for (i = 0; i <= products.length; i++) {
    productNames.innerHTML += `
    <div id="card-body-mdiv" class="card" style="width: 18rem;">
  <div id="card-body-cdiv" class="card-body">
    <h2 class="card-title">${products[i].name}</h2>
    <p class="card-text">${products[i].description}</p>
  </div>
  <ul id="card-ul" class="list-group list-group-flush">
    <li class="list-group-item">${products[i].price}</li>
  </ul>
  <div class="card-body">
    <button id="card-btn"><a href="#" class="card-link">Card link</a></button>
  </div>
</div>`
}